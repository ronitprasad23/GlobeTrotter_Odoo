...
export default function TripDetails() {
  ...
  const fetchTripDetails = () => {
    fetch(`http://127.0.0.1:8000/api/trips/${id}/`, {
      ...
      .then(data => {
        setTrip(data);
        if (data.start_date) {
          setActivityDate(data.start_date);
          setSelectedCalendarDate(data.start_date);
        }
      })
      .catch(err => console.error(err));
  };
  ...
  const handleAddCity = (e) => {
    ...
    fetch('http://127.0.0.1:8000/api/cities/', {
      ...
      body: JSON.stringify({
        ...
        region: newCityRegion || 'Global',
        cost_index: Number(newCityCost),
        popularity: Number(newCityPopularity),
        image_url: newCityImageUrl || ''
      })
    })
      .then(res => {
        if (!res.ok) throw new Error('Failed to register city');
        return res.json();
      })
      .then(newCity => {
        setDbCities(prev => [...prev, newCity]);
  ...
  // Merge database cities with preset POPULAR_CITIES
  const POPULAR_CITIES = [
    { name: 'Paris', country: 'France', activities: ['Eiffel Tower Climb', 'Louvre Museum Tour', 'Seine River Cruise'] },
    { name: 'Tokyo', country: 'Japan', activities: ['Shibuya Crossing Walk', 'Senso-ji Temple Visit', 'Robot Restaurant Show'] },
    { name: 'Goa', country: 'India', activities: ['Anjuna Beach Sunset', 'Fort Aguada Exploration', 'Basilica of Bom Jesus'] },
    { name: 'Rome', country: 'Italy', activities: ['Colosseum Guided Tour', 'Vatican Museums', 'Trevi Fountain Coin Toss'] },
    { name: 'Mumbai', country: 'India', activities: ['Gateway of India', 'Marine Drive Walk', 'Elephanta Caves Tour'] }
  ];
  ...
  const filteredStopCities = mergedCities.filter(c => {
  ...
  const currentStopCityName = trip.stops?.find(s => s.id === selectedStopIdForActivity)?.city?.name || 
                              trip.stops?.find(s => s.id === selectedStopIdForActivity)?.city_name;
  
  const matchedPresetCity = POPULAR_CITIES.find(pc => pc.name.toLowerCase() === currentStopCityName?.toLowerCase());
  const presetActivities = matchedPresetCity 
    ? matchedPresetCity.activities.map((actName, idx) => ({
        id: `preset-${idx}`,
        name: actName,
        description: `Suggested activity in ${matchedPresetCity.name}`,
        activity_type: 'Sightseeing',
        estimated_cost: 0
      }))
    : [];

  const dbActivitiesForCity = dbCities.find(c => c.id === currentStopCityId)?.activities || [];
  
  const combinedActivities = [...presetActivities];
  dbActivitiesForCity.forEach(da => {
    if (!combinedActivities.some(a => a.name.toLowerCase() === da.name.toLowerCase())) {
      combinedActivities.push(da);
    }
  });

  const filteredActivitiesForSearch = combinedActivities.filter(act => {
    ...
    const matchesCost = activityCostFilter === 'All' ||
      (activityCostFilter === 'Free' && (act.estimated_cost === 0 || !act.estimated_cost)) ||
            ...
            <div className="space-y-6 relative before:absolute before:inset-y-2 before:left-[19px] before:w-0.5 before:bg-gradient-to-b before:from-primary-500 before:to-amber-500">
              {(!trip.stops || trip.stops.length === 0) ? (
                ...
                trip.stops.map((stop, index) => {
                  const city = stop.city || citiesList.find(c => c.id === stop.city_id);
                  const stopActivities = (stop.itinerary_items || []).sort((a, b) => (a.start_time || '').localeCompare(b.start_time || ''));
                  return (
                    <div 
                      key={stop.id} 
                      draggable
                    ...
                    >
                      ...
                      <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 hover:shadow-md transition-shadow">
                        ...
                        {(() => {
                          const matchedPreset = POPULAR_CITIES.find(pc => pc.name.toLowerCase() === city?.name?.toLowerCase());
                          const cityPresets = matchedPreset ? matchedPreset.activities : [];
                          ...
                          return (
                            <div className="mt-4 bg-primary-50/20 p-4 rounded-xl border border-primary-100/30">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {cityPresets.map((presetName, idx) => {
                                  const isScheduled = stopActivities.some(sa => (sa.activity_detail?.name || sa.activity_name || '').toLowerCase() === presetName.toLowerCase());
                                  return (
                                    <button
                                      key={idx}
                                      type="button"
                                      disabled={isScheduled}
                                      onClick={() => handleInstantAddActivity(stop.id, stop.start_date, `preset-${idx}`, presetName, 0)}
