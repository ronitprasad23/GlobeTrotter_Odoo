    ...
    <div className="bg-gray-50/50 min-h-screen pb-12">
      ...
      >
        <div className="max-w-4xl mx-auto px-4 text-center space-y-6">
          ...
          <p className="text-xl md:text-2xl text-gray-100 max-w-3xl mx-auto font-semibold drop-shadow-sm leading-relaxed">
          ...
          <div className="max-w-2xl mx-auto bg-white rounded-full shadow-lg p-2.5 flex items-center border border-gray-100 mt-10 text-gray-800">
            <Search className="h-6 w-6 text-gray-405 ml-4 shrink-0" />
            <input
              type="text"
              placeholder="Search your planned trips..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="flex-1 px-4 py-2.5 text-base focus:outline-none placeholder-gray-400 bg-transparent font-medium"
            />
            <Link
              to="/trips/create"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-12 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <div className="bg-white p-7 rounded-2xl border border-gray-100 shadow-md flex items-center hover:translate-y-[-2px] transition-transform duration-200">
            <div className="p-4 bg-primary-50 text-primary-500 rounded-xl mr-5 shadow shadow-primary-500/5">
              <Compass className="h-7 w-7" />
            </div>
            <div>
              <span className="text-xs text-gray-450 font-bold uppercase tracking-wider block">Trips Planned</span>
              <span className="text-2xl font-black text-gray-800">{totalTrips} Trips</span>
            </div>
          </div>
          <div className="bg-white p-7 rounded-2xl border border-gray-100 shadow-md flex items-center hover:translate-y-[-2px] transition-transform duration-200">
            <div className="p-4 bg-emerald-50 text-emerald-600 rounded-xl mr-5 shadow">
              <DollarSign className="h-7 w-7" />
            </div>
            <div>
              <span className="text-xs text-gray-450 font-bold uppercase tracking-wider block">Estimated Budget</span>
              <span className="text-2xl font-black text-gray-800">₹{totalBudget.toLocaleString()}</span>
            </div>
          </div>
          <div className="bg-white p-7 rounded-2xl border border-gray-100 shadow-md flex items-center hover:translate-y-[-2px] transition-transform duration-200">
            <div className="p-4 bg-amber-50 text-amber-600 rounded-xl mr-5 shadow">
              <TrendingUp className="h-7 w-7" />
            </div>
            <div>
              <span className="text-xs text-gray-450 font-bold uppercase tracking-wider block">Logged Expense</span>
        ...
        <div className="mb-12">
          <div className="flex justify-between items-end mb-6">
            <div>
              <h2 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tight">Your Plan Dashboard</h2>
              <p className="text-sm text-gray-500 mt-1">Pick up planning where you left off</p>
            </div>
          ...
          {recentTrips.length === 0 ? (
            <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center shadow-sm">
              <Compass className="h-14 w-14 text-gray-300 mx-auto mb-4" />
                ...
                return (
                  <div key={trip.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col justify-between hover:shadow-md transition-shadow">
                    <div className="p-6 md:p-8">
                      ...
                      <div className="space-y-2.5 text-sm md:text-base text-gray-500 mb-6">
                        <div className="flex items-center font-bold">
                          <Calendar className="h-4 w-4 mr-2 text-primary-500 shrink-0" />
                          {trip.start_date} to {trip.end_date}
                        </div>
                        <div className="flex items-center font-bold">
                          <MapPin className="h-4 w-4 mr-2 text-brandTeal-600 shrink-0" />
                          {stopsCount} Stop{stopsCount !== 1 && 's'} Planned
                        </div>
                      </div>
                      <div className="w-full bg-gray-100 rounded-full h-3 mb-3">
                        <div 
                          className={`h-3 rounded-full ${spent > trip.budget ? 'bg-red-655' : 'bg-primary-500'}`}
                          style={{ width: `${pct}%` }}
                        ></div>
                      </div>
                      <div className="flex justify-between text-xs md:text-sm text-gray-500 font-extrabold">
                        <span>Spent: ₹{spent.toLocaleString()}</span>
                        <span>Budget: ₹{trip.budget.toLocaleString()}</span>
                      </div>
                    </div>
                    <div className="bg-gray-50 px-6 md:px-8 py-4 border-t border-gray-100 flex justify-end">
                      <Link to={`/trips/${trip.id}`} className="text-sm font-black text-primary-500 hover:text-primary-600">
        ...
        <div>
          <div className="mb-6">
            <h2 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tight">Explore Recommended Destinations</h2>
            <p className="text-sm text-gray-500 mt-1">Akshar Travels popular domestic & international packages</p>
          </div>
            ...
            {popularCities.map((city) => (
              <div key={city.name} className="group bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                <div className="h-48 overflow-hidden relative">
                  ...
                  <span className="absolute top-3 left-3 bg-white/95 backdrop-blur-sm text-gray-800 text-[10px] uppercase tracking-wider font-extrabold px-2.5 py-1 rounded-full shadow-sm">
                    {city.cost} Cost
                  </span>
                ...
                <div className="p-5">
                  ...
                  <p className="text-gray-500 text-sm mt-1">{city.country}</p>
                </div>
              </div>
            ))}
          </div>
